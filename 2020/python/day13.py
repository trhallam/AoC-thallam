...
def find_my_bus(input):
    earliest, buss, _ = parse_input(input)
    print(earliest, buss)
    wait = [-1 * (earliest % bus) + bus for bus in buss]
    for bus, w in zip(buss, wait):
        print(bus, w, bus * w)


# Wikipedia: Chinese Remainder Theorem
def find_time(input):
    _, buss, delays = parse_input(input)
    options = sorted([(i, offset) for offset, i in zip(delays, buss)])
    inc, off = options.pop()
    next_time = inc - off
    print(next_time, inc, off)
...
if __name__ == "__main__":
    ...
    # ms = find_sequence2(test)
    # ms = find_sequence3(test, 100)
    ms = find_time(test)
    ...
    # print("Answer 1:", find_my_bus(mt))
    print("Answer 2:", find_time(mt))
    # find_sequence(mt)